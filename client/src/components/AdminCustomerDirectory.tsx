...
import { ArrowDownUp, Filter, Search, SlidersHorizontal, UserRoundPlus } from "lucide-react";
...
export default function AdminCustomerDirectory({ adminFetch, enabled }: { adminFetch: AdminFetch; enabled: boolean }) {
  ...
  const takeUser = useMutation({ mutationFn: (userId: string) => adminFetch(`/api/admin/crm/users/${userId}/take`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ownerId: Number(activeAdminId) }) }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/users"] }); queryClient.invalidateQueries({ queryKey: ["/api/admin/crm/contacts"] }); } });
  ...
  return <div className="space-y-5">
    ...
    <Card><CardContent className="p-0">{isLoading ? <p className="p-6 text-sm text-muted-foreground">Загружаем базу...</p> : <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-sm"><thead className="border-b bg-muted/40 text-left text-xs text-muted-foreground"><tr><th className="p-3">Клиент</th><th className="p-3">Контакты</th><th className="p-3">Источник</th><th className="p-3 text-right">XP</th><th className="p-3 text-right">Заказы</th><th className="p-3">Последняя покупка</th><th className="p-3">В работе</th><th className="p-3"></th></tr></thead><tbody>{filtered.map((user) => <tr key={user.id} className="border-b last:border-0 hover:bg-muted/30"><td className="p-3"><p className="font-medium">{user.name || "Без имени"}</p><p className="max-w-28 truncate text-xs text-muted-foreground" title={user.id}>{user.id}</p></td><td className="p-3"><p>{user.phone}</p><p className="text-xs text-muted-foreground">{user.email || "-"}</p></td><td className="p-3">{user.source || "-"}</td><td className="p-3 text-right">{user.xp}</td><td className="p-3 text-right">{user.orderCount} · {user.totalSpent.toLocaleString("ru-RU")} ₽</td><td className="p-3">{dateFormat(user.lastOrderAt)}</td><td className="p-3">{user.ownerName ? <><p>{user.ownerName}</p><p className="text-xs text-muted-foreground">{user.workStatus === "in_progress" ? "в работе" : user.workStatus}</p></> : <span className="text-muted-foreground">Не назначен</span>}</td><td className="p-3 text-right"><Button size="sm" variant={user.ownerId === Number(activeAdminId) ? "outline" : "default"} disabled={!activeAdminId || takeUser.isPending} onClick={() => takeUser.mutate(user.id)}>{user.ownerId === Number(activeAdminId) ? "У меня" : "Взять в работу"}</Button></td></tr>)}</tbody></table>{filtered.length === 0 && <p className="p-6 text-sm text-muted-foreground">Подходящих пользователей нет.</p>}</div>}</CardContent></Card>
