...
type Filters = { search: string; source: string; minXp: string; maxXp: string; minBalance: string; maxBalance: string; verified: string; discount: string; firstDiscount: string; orderAge: string; analytics: string };
const EMPTY: Filters = { search: "", source: "all", minXp: "", maxXp: "", minBalance: "", maxBalance: "", verified: "all", discount: "all", firstDiscount: "all", orderAge: "all", analytics: "" };
...
export default function AdminCustomerDirectory({ adminFetch, enabled }: { adminFetch: AdminFetch; enabled: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((user) => {
      ...
      if (filters.orderAge === "over45" && !(age >= 45)) return false;
      return true;
    }).sort((a, b) => {
