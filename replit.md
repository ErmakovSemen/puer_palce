...
-   **Telegram Bot**: Customer-facing bot with commands `/start`, `/help`, `/contacts`, `/menu`, `/profile`. Features inline keyboard navigation, contact info from site_settings, and loyalty status display for linked accounts. Webhook endpoint at `/api/telegram/webhook`. Database tables: `telegram_profiles` (links Telegram chat_id to user accounts), `magic_links` (one-time authentication tokens). Phase 2 includes magic-link authentication for secure account binding from user profile page with deep link support.
