...
- **Password Reset**: Complete forgot password flow with 6-digit verification codes sent via email, valid for 15 minutes. Users enter their email on `/forgot-password`, receive a code, and create a new password on `/reset-password` with password confirmation validation. Security: All error responses use identical generic messages to prevent user enumeration attacks (attackers cannot determine if an email is registered). Development mode logs codes to console. Resend button has 30-second cooldown timer. Password must be minimum 6 characters.
