...
-   **Info Banners System**: Customizable informational banners with visual drag-and-drop admin editor. Features: dark/light themes, optional icons (Lucide), action buttons, separate desktop/mobile slot configurations (after_header, after_filters, before_products, between_products, after_products, before_footer), visibility toggles per device, ordering within slots, and size variants (width: quarter/half/threeQuarter/full; height: compact/standard/tall). Visual editor uses @dnd-kit for accessible drag-and-drop with schematic page preview showing all banner slots. Between-products slot supports row index positioning (0-4) for placing banners after specific product rows. CSS-based responsive rendering prevents layout flash. Admin tab: "Баннеры".
