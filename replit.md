...
-   **Telegram Bot**: Customer-facing bot (@PuerPabbot) with commands `/start`, `/help`, `/contacts`, `/menu`, `/profile`. Features inline keyboard navigation, contact info from site_settings, loyalty status display with visual progress bar for linked accounts. Phase 2: magic-link authentication for secure account binding from user profile page with deep link support and auto-refresh tokens. Phase 3: product catalog browsing by category (tea/teaware), detailed product views with pricing and effects, enhanced profile display. Webhook endpoint at `/api/telegram/webhook`. Database tables: `telegram_profiles` (links Telegram chat_id to user accounts), `magic_links` (one-time authentication tokens).
